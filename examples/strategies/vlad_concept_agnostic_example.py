...
if __name__ == "__main__":
    """
    VLAD in the concept-agnostic scenario: no task/concept boundaries are provided, so the
    ...
    # for_concept_agnostic() defaults to ExactWassersteinDistance: this scenario scans tiny
    # mini_batch_size-sized chunks (WassersteinChangePointDetector), where the faster
    # SlicedWassersteinDistance is empirically unreliable — see its docstring.
    ...
    strategy = VladConceptAgnosticStrategy(model=model, memory=memory, max_steps_between_updates=5000)
    ...
    scenario = ConceptAgnosticScenario(dataset, strategy=strategy, callbacks=callbacks)
    scenario.run()
