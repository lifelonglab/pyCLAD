...
def _supervisely_bitmap_mask(bitmap: Optional[dict], canvas_shape: tuple[int, int]) -> np.ndarray:
...
def _polygon_mask(points: Optional[dict], canvas_shape: tuple[int, int]) -> np.ndarray:
...
def _rectangle_mask(points: Optional[dict], canvas_shape: tuple[int, int]) -> np.ndarray:
    if not isinstance(points, dict):
