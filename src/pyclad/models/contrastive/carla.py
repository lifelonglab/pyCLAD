...
class Carla(Model):
    ...
    def _eval_batch(
        ...
        """Computes the classification loss for one batch of anchor/nearest/furthest triplets."""
        anchors = windows[batch_idx]
        nearest = windows[nn_idx[batch_idx]]
        furthest = windows[fn_idx[batch_idx]]
        batch_size, n_neighbours, window_size, feature_dimension = nearest.shape
        anchor_probs = module(anchors)
        nearest_probs = module(nearest.reshape(batch_size * n_neighbours, window_size, feature_dimension)).reshape(
            batch_size, n_neighbours, -1
        )
        furthest_probs = module(furthest.reshape(batch_size * n_neighbours, window_size, feature_dimension)).reshape(
            batch_size, n_neighbours, -1
        )
    ...
    def _classification_stage(
        ...
        for epoch in range(self._classification_epochs):
            ...
            for start in range(0, n_train, self._batch_size):
                ...
                loss = self._eval_batch(module, train_windows, train_nn_idx, train_fn_idx, batch_idx)
                optimizer.zero_grad()
                loss.backward()
                optimizer.step()
                total_loss += loss.item() * batch_idx.numel()
                n_samples += batch_idx.numel()
            ...
            with torch.no_grad():
                for start in range(0, n_val, self._batch_size):
                    ...
                    val_loss = self._eval_batch(module, val_windows, val_nn_idx, val_fn_idx, batch_idx)
                    val_total += val_loss.item() * batch_idx.numel()
                    val_samples += batch_idx.numel()
