...
DEFAULT_VISION_DATASET_REGISTRY_PATH = (
    Path(__file__).resolve().parents[2] / "datasets" / "vision_datasets" / "registry.json"
)
DEFAULT_VISION_DATASETS_ROOT = Path(__file__).resolve().parents[5] / "examples" / "resources" / "vision"
...
def _raise_missing_vision_benchmark_root(
    ...
    message = _build_missing_vision_benchmark_root_message(
        benchmark_name=benchmark_name,
        registry_path=registry_path,
        attempted_paths=attempted_paths,
...
def _build_missing_vision_benchmark_root_message(
    benchmark_name: str,
    registry_path: Optional[Union[str, Path]],
    attempted_paths: Sequence[Path],
    explicit_root: Optional[Path] = None,
    ...
    message_lines = [
        f"Could not resolve local root for vision benchmark '{benchmark_name}'.",
    ]
    ...
    if attempted_paths:
        ...
        message_lines.append(f"Checked paths: {attempted}")
