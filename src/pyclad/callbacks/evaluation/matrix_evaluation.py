...
class MatrixMetricEvaluationCallback(Callback, InfoProvider):
    ...
    def after_evaluation(self, evaluated_concept: Concept, y_true: np.ndarray, y_pred: np.ndarray,
                         anomaly_scores: np.ndarray, *args, **kwargs):
        assert evaluated_concept.name not in self._metric_matrix[
            self._learned_concepts[-1]], "The same concept should not be evaluated twice after the same learned concept"
    ...
    def info(self) -> Dict[str, Any]:
        ...
        return {
            f'matrixMetricEvaluationCallback_{self._base_metric.name()}': {
                self._base_metric.name(): {'metrics': lifelong_learning_metrics,
                                           'concepts_order': self._learned_concepts,
                                           'matrix': self._metric_matrix}}
        }

    @staticmethod
    def _transform_to_ordered_matrix(metric_matrix: Dict[str, Dict[str, float]],
                                     concepts_order: List[str]) -> ConceptLevelMatrix:
        values = []
